...
int ScsiCmdSend(BYTE Cmd, BYTE SubCommand, SCSI_BULK_DIRECTION bIn_Out, ULONG DataTransferLength, UCHAR *databuf) {
    ...
    if(bIn_Out == SCSI_BULK_IN)  // bulk in
    {
        ...
        status           = DeviceIoControl(m_DeviceHandle, IOCTL_SCSI_PASS_THROUGH, &sptwb, length, &sptwb, length, &returned, FALSE);
        /*
        sptwb.spt.Cdb[0] = Cmd;
        sptwb.spt.Cdb[1] = SubCommand;
        sptwb.spt.Cdb[6]  = (BYTE)((DataTransferLength) >> 24);
...
BOOL USB_ScsiInquiry(UCHAR *ucDataBuf) {
    ...
    sptwb.spt.SenseInfoOffset    = offsetof(SCSI_PASS_THROUGH_WITH_BUFFERS, ucSenseBuf);
    length                       = offsetof(SCSI_PASS_THROUGH_WITH_BUFFERS, ucDataBuf) + sptwb.spt.DataTransferLength;
    ret                          = DeviceIoControl(m_DeviceHandle, IOCTL_SCSI_PASS_THROUGH, &sptwb, sizeof(SCSI_PASS_THROUGH), &sptwb, length, &returned, FALSE);
...
HANDLE USB_ScsiRefindDevice(int *dev_state) {
    ...
    strcpy_s(diskname, sizeof(diskname) / sizeof(TCHAR), src);
...
HANDLE USB_ScsiFindDevice(int *dev_state,int *disk_number,std::set<int> disk_number_set) {
    ...
    for(int i = 0x44; i < 0x5B; i++) {
        diskname[4] = i;
        if(_ScsiFindDevice(diskname, dev_state) && disk_number_set.find(i) == disk_number_set.end()) {
            m_DeviceDisk = i;
            *disk_number = i;
            return m_DeviceHandle;
        }
